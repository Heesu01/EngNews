...
import { fetchNaverCategories } from "../api/CategoryApi";
import {
  fetchNaverTop5News,
  fetchNaverArticlesByCategory,
  fetchNaverArticlesByKeyword,
} from "../api/NewsApi";
...
const NaverNews = () => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await fetchNaverCategories();
        setCategories([
  ...
  const fetchArticles = useCallback(async () => {
    ...
    try {
      ...
      if (selectedCategory === "Top5") {
        response = await fetchNaverTop5News();
        setHasMore(false);
      } else if (selectedCategory === "키워드 맞춤") {
        response = await fetchNaverArticlesByKeyword(selectedSort);
        setHasMore(response.data?.length > 0);
      } else {
        response = await fetchNaverArticlesByCategory(
          selectedCategory,
          page,
          selectedSort
        );
      ...
      <BottomContainer>
        <SearchContainer>
          ...
          <SortContainer>
            ...
            <p>|</p>
            <SortOption
              selected={selectedSort === "sim"}
              onClick={() => selectSortOption("sim")}
            >
              정확도순
            </SortOption>
          </SortContainer>
